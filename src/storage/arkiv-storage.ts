...
export class ArkivStorage implements IStorage {
  ...
  async storeBatchChunks(chunks: ChunkEntity[]): Promise<string[]> {
    ...
    try {
      ...
      const creates = chunks.map((chunk) =>
        this.buildChunkPayload(
          chunk,
          this.blocksToSeconds(Math.max(chunk.expiration_block - currentBlock, 1)),
        ),
      )

      console.log(
        `🚀 Batch storing ${creates.length} chunk entities for file ${chunks[0]?.file_id}`,
      )
      ...
      result.createdEntities.forEach((entityKey, index) => {
        ;(chunks[index] as ChunkEntity).entity_key = entityKey
      })
  ...
  async storeBatch(
    ...
    try {
      ...
      const metadataPayload = this.buildMetadataPayload(
        metadata,
        this.blocksToSeconds(Math.max(metadata.expiration_block - currentBlock, 1)),
      )
      ...
      console.log(
        `🚀 Batch storing ${creates.length} entities (1 metadata + ${chunks.length} chunks) for file ${metadata.file_id}`,
      )
      ...
      chunkKeys.forEach((key, index) => {
        ;(chunks[index] as ChunkEntity).entity_key = key
      })

      console.log(
        `✅ Batch stored file ${metadata.file_id} with ${chunks.length} chunks in single transaction`,
      )

      return { metadata_key: result.createdEntities[0] as string, chunk_keys: chunkKeys }
    } catch (error) {
  ...
  async getChunk(file_id: string, chunk_index: number): Promise<ChunkEntity | null> {
    ...
    try {
      ...
      const query = publicClient
        ...
        .withAttributes(true)
        .withPayload(true)
        .where([eq("type", "chunk"), eq("file_id", file_id), eq("chunk_index", chunk_index)])
        .limit(1)
      ...
      const entity = result.entities[0] as Entity
    ...
    try {
      if (metadataEntityKey) {
        try {
          ...
          console.warn(
            `⚠️ Failed to get metadata with entity key ${metadataEntityKey}, falling back to full scan`,
          )
        }
      }
      ...
      const query = publicClient
        ...
        .withAttributes(true)
        .withPayload(true)
        .where([eq("type", "metadata"), eq("file_id", file_id)])
        .limit(1)
  ...
  async getAllChunks(file_id: string, chunkEntityKeys?: string[]): Promise<ChunkEntity[]> {
    ...
    try {
        ...
        return entities
          .map(({ entity, index }) => {
            ...
            return {
              ...
              checksum: String(attributeMap.get("checksum") ?? ""),
              created_at: new Date(
                String(attributeMap.get("created_at") ?? new Date().toISOString()),
              ),
              expiration_block: Number(attributeMap.get("expiration_block") ?? 0),
              entity_key: entity.key,
            }
      ...
      const query = publicClient
        ...
        .where([eq("type", "chunk"), eq("file_id", file_id)])
  ...
  async getUserQuota(userAddress: string): Promise<{ used_bytes: number; uploads_today: number }> {
    ...
    try {
      ...
      const query = publicClient
        ...
        .withAttributes(true)
        .withPayload(true)
        .where([eq("type", "quota"), eq("user_address", userAddress), eq("date", today)])
        .limit(1)
      ...
      const payload = result.entities[0]?.payload
        ? Buffer.from(result.entities[0].payload)
        : Buffer.from("{}")
    ...
    try {
      ...
      const payload: ArkivEntityCreatePayload = {
        ...
        attributes: [
          ...
          { key: "date", value: today as string },
  ...
  async getFilesByOwner(owner: string): Promise<FileMetadata[]> {
    ...
    try {
      ...
      const query = publicClient
        ...
        .withAttributes(true)
        .withPayload(true)
        .where([eq("type", "metadata"), eq("owner", owner)])
  ...
  private parseMetadataPayload(entity: {
    payload?: Uint8Array
    attributes: Attribute[]
  }): FileMetadata {
    ...
    return {
      ...
      expiration_block: Number(
        data.expiration_block ||
          this.attributesToMap(entity.attributes).get("expiration_block") ||
          0,
      ),
      btl_days: Number(
        data.btl_days || this.attributesToMap(entity.attributes).get("btl_days") || 0,
      ),
      owner: data.owner,
    }
  }
  ...
  private async retryBlockchainOperation<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `⚠️ Attempt ${attempt} failed for ${operationName}, retrying in ${delay}ms:`,
