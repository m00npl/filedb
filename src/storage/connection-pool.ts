...
export class ArkivDBConnectionPool {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      `✅ Connection pool initialized: ${this.writeConnections.length} write, ${this.readConnections.length} read`,
    )
  }
  ...
  async executeWithWriteConnection<T>(
    operation: (client: WalletArkivClient) => Promise<T>,
  ): Promise<T> {
  ...
  async executeWithReadConnection<T>(
    operation: (client: PublicArkivClient) => Promise<T>,
  ): Promise<T> {
  ...
  private async executeWithRetry<T>(operation: () => Promise<T>): Promise<T> {
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      try {
        ...
        console.warn(
          `⚠️ Connection operation failed (attempt ${attempt}), retrying in ${delay}ms:`,
          error,
        )
  ...
  private async waitForConnection(
    waitQueue: ((connection: PooledConnection) => void)[],
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    if (stats.totalConnections > 0) {
      console.log(
        `🔍 Pool health: ${stats.writeActive}/${stats.writeTotal} write, ${stats.readActive}/${stats.readTotal} read active`,
      )
    }
  }
  ...
  async shutdown(): Promise<void> {
    ...
    this.waitingForWrite.forEach((resolve) => {
      resolve({} as PooledConnection)
    })
    this.waitingForRead.forEach((resolve) => {
      resolve({} as PooledConnection)
    })
