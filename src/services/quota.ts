...
import { CONFIG, type QuotaInfo } from "../types"
...
export class QuotaService {
  ...
  private shouldResetQuota(userId: string): boolean {
    ...
    const today: string = todayParts[0] ?? new Date().toISOString().slice(0, 10)
  ...
  async updateUsage(userId: string, fileSize: number): Promise<void> {
    ...
    if (this.storage?.updateUserQuota) {
      const timeoutMs = 30000 // 30 second timeout
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error("Blockchain quota update timed out")), timeoutMs)
      })

      Promise.race([this.storage.updateUserQuota(userId, fileSize), timeoutPromise]).catch(
        (error) => {
  ...
  private async getUserQuota(userId: string): Promise<QuotaInfo> {
    ...
    if (this.cacheInitialized) {
      try {
        ...
        console.warn(
          "Redis quota cache read failed:",
          error instanceof Error ? error.message : String(error),
        )
      }
    }

    if (this.storage?.getUserQuota) {
      ...
      try {
        ...
        if (this.cacheInitialized) {
          try {
            ...
            console.warn(
              "Redis quota cache write failed:",
              error instanceof Error ? error.message : String(error),
            )
          }
        }
    ...
    const quota = this.userUsage.get(userId)
    if (!quota) {
      // This should never happen since we just set it above, but TypeScript needs this
