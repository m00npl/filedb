...
export class ParallelUploadService implements UploadWorkerPool {
  ...
  async processChunks(
    ...
    console.log(
      `✅ Parallel upload completed: ${results.length - failures.length}/${results.length} batches successful`,
    )
  }
  ...
  private async uploadBatchWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `📦 Batch ${batchIndex + 1}: uploading ${batch.length} chunks (attempt ${attempt}/${maxRetries})`,
        )
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.activeUploads.size > 0) {
      console.log(
        `🛑 Shutting down parallel upload service (${this.activeUploads.size} active uploads)`,
      )
