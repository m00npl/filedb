...
import { fileUploadValidationMiddleware, inputValidationMiddleware } from "@/middleware/validation"
...
async function initializeServices() {
  ...
  await uploadService.initialize(storage)
...
app.get(
  "/files/by-owner/:owner",
  optionalJwtAuthMiddleware,
  requirePermission(PERMISSIONS.LIST_FILES),
  async (c) => {
    try {
...
app.get(
  "/files/:file_id",
  optionalJwtAuthMiddleware,
  requirePermission(PERMISSIONS.DOWNLOAD_FILES),
  async (c) => {
    try {
  ...
  async (c) => {
    try {
      ...
      return c.json({
        ...
        expires_at: new Date(
          metadata!.created_at.getTime() + metadata!.btl_days * 24 * 60 * 60 * 1000,
        ),
...
app.get(
  "/quota",
  optionalJwtAuthMiddleware,
  requirePermission(PERMISSIONS.VIEW_QUOTA),
  async (c) => {
    try {
      const userId = quotaService.getUserId(c.req)
    ...
    try {
      ...
      const progress = {
        ...
        percentage: Math.round(
          (session.chunks_uploaded_to_blockchain / session.total_chunks) * 100,
        ),
